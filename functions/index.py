...
def create_gradio_app(main_loop, image_path=default_image_file_path):
    with gr.Blocks(theme="soft") as demo:
        ...
        app = gr.routes.AppManager.from_blocks(demo)
    return app


# Initialize the Gradio app outside of the function
app = create_gradio_app(main_loop, default_image_file_path)

# Existing history initialization
history = [{'role': 'system', 'content': system_prompt}]

@https_fn.on_request(
    memory=options.MemoryOption.MB_1_GIB,
    cpu=options.CpuOption.CPU_1,
)
def gradio(req: https_fn.Request) -> https_fn.Response:
    return app.get_blocks_response(req)
